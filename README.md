...
2) Put the files you want to crop to the input directory in script root,
   or, alternatively,  
   put the files into any directory and path it's root as argument.  
3) Choose yours preffered margins (boundaries) for pages in pdfbook.sh  
4) Modify the rights for script execution:  
        chmod u+x /path/to/pdfbook.sh  
4) Pass your input or use embedded with script input directory:  
